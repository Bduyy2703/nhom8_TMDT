...
public static void updateInvoiceById(Invoice invoice) {
    EntityManager em = DBUtil.getEmFactory().createEntityManager();
    EntityTransaction trans = em.getTransaction();
    
    try {
        trans.begin();
        em.merge(invoice); // Cập nhật thông tin của invoice trong cơ sở dữ liệu
        trans.commit(); // Commit transaction sau khi cập nhật thành công
    } catch (Exception e) {
        if (trans.isActive()) {
            trans.rollback(); // Rollback transaction nếu có lỗi xảy ra
        }
